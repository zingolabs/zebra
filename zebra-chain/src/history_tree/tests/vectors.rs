...
fn push_and_prune() -> Result<()> {
    ...
    push_and_prune_for_network_upgrade(Network::public_testnet(), NetworkUpgrade::Heartwood)?;
    push_and_prune_for_network_upgrade(Network::Mainnet, NetworkUpgrade::Canopy)?;
    push_and_prune_for_network_upgrade(Network::public_testnet(), NetworkUpgrade::Canopy)?;
    Ok(())
}
...
fn upgrade() -> Result<()> {
    ...
    upgrade_for_network_upgrade(Network::public_testnet(), NetworkUpgrade::Canopy)?;
    Ok(())
}
